...
private:
    ...
    static int size_impl(const Node* node) {
        // BASE CASE ¨C Empty tree has size 0
        if (!node) {
            return 0;
        }
        // RECURSIVE CASE
    ...
    static Node* copy_nodes_impl(Node* node) {
        // BASE CASE - empty tree returns nullptr
        if (!node) {
            return nullptr;
        }
        // RECURSIVE CASE
        Node* new_root = new Node(node->datum,
    ...
    static Node* find_impl(Node* node, const T& query, Compare less) {
        // BASE CASE 1 - empty tree returns nullptr
        if (!node) {
            return nullptr;
        }
        // BASE CASE 2 - element is found, returns node
    ...
    static Node* max_element_impl(Node* node) {
        ...
        // BASE CASE 2 - most right element
        if (!node->right) {
            return node;
        }
        // RECURSIVE CASE
    ...
    static bool check_sorting_invariant_impl(const Node* node, Compare less) {
        // BASE CASE 1 - empty tree, returns true
        if (!node) {
            return true;
        }
    ...
    static Node* min_greater_than_impl(Node* node, const T& val, Compare less) {
        ...
        if (less(val, node->datum)) {
            ...
            else if (!(less(val, node->left->datum))) {
                return node;
            }
            else {
