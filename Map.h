...
private:
    ...
    class PairComp {
    public:
        // I don't know how to write the constructor: PairComp() 
...
public:
  ...
  //    // default constructor
  //    Map() : entries(BinarySearchTree<Pair_type, PairComp>()) {}
  //
  //    // destructor
  //    ~Map() {
  //        ~BinarySearchTree<Pair_type, PairComp>();
  //    }
  //
  //    // copy constructor
  //    Map(const Map& other)
  //    : entries(BinarySearchTree<Pair_type, PairComp>(other.entries)) {}
  //
  //    // overloaded assignment operator
  //    Map &operator=(const Map &rhs) {
  //        // question ???
  //        entries = entries.BinarySearchTree<Pair_type, PairComp>(rhs.entries);
  //        return *this;
  //    }
    ...
    Value_type& operator[](const Key_type& k) {
        ...
        if (result == end()) {
            ...
            return (entries.insert({ k, Value_type() }))->second;
        }
        else {
    ...
    std::pair<Iterator, bool> insert(const Pair_type& val) {
        Iterator result = entries.find(val);
        if (result == entries.end()) {
            Iterator iter = entries.insert(val);
            return { iter, true };
        }
        else {
            return { result, false };
